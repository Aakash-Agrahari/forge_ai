...
export async function discoverAllModels() {
    ...
    for (const provider of configuredProviders) {
        ...
        if (!DISCOVERY_STRATEGIES.has(provider.discovery)) {
            results.push({
                provider: provider.id,
                status: "unsupported_strategy",
                models: [],
                error: `Unsupported discovery strategy: ${provider.discovery}`
            });
        ...
        try {
            ...
            if (!Array.isArray(models)) {
                throw new Error(
                    `Discovery handler for ${provider.id} must return an array`
                );
            }
